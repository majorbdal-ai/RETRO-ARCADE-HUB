...
function launchOrig2048() {
  ...
  // hide any other game's stage so only 2048 shows (prevents game-bleed)
  const nfStage = document.getElementById('neonflapStage');
  const nfFrame = document.getElementById('neonflapFrame');
  const nfStats = document.getElementById('neonflapStats');
  if (nfStage) nfStage.style.display = 'none';
  if (nfFrame) nfFrame.style.display = 'none';
  if (nfStats) nfStats.style.display = 'none';
  if (frame) {
...
function launchOrigNeonFlap() {
  ...
  if (canvas) canvas.style.display = 'none';
  if (stage) stage.style.display = 'flex';
  // hide any other game's stage so only neon-flap shows (prevents game-bleed)
  const oStg = document.getElementById('orig2048Stage');
  const oFrm = document.getElementById('orig2048Frame');
  const oStt = document.getElementById('orig2048Stats');
  if (oStg) oStg.style.display = 'none';
  if (oFrm) oFrm.style.display = 'none';
  if (oStt) oStt.style.display = 'none';
