...
> A premium, cyberpunk-themed retro arcade gaming hub with **70 fully playable HTML5 canvas games**, 6 dynamic themes, per-game skins, XP/levels, achievements, daily quests, coins/shop, leaderboard, and offline PWA support.
...
| Area | Details |
|---|---|
| **Games** | 70 arcade games — action, puzzle, sports, Bangladesh viral, classic remakes. Every game = canvas engine with score/coins/restart |
...
| **PWA** | Offline-first service worker (pre-caches all 70 engines), install prompt, standalone display, shortcuts |
...
- **GitHub Actions** — CI (syntax + 70-game headless lifecycle + registry consistency) + auto-deploy to Pages

## 🧪 Testing
```bash
node test/run_all.js      # headless lifecycle test — all 70 engines
...
games/<game>.js        # individual game engines (70)
